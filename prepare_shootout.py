...
"""
USAGE: %(program)s WIKI_DUMP.xml.bz2 OUTPUT_DIRECTORY

Parse articles from a Wikipedia dump, storing several files ito OUTPUT_DIRECTORY:

* title_tokens.txt.gz: raw article titles and tokens, one article per line, "title[TAB]space_separated_tokens[NEWLINE]"
* dictionary: pickled dictioanry object containing the word<=>word_id mapping
...
def process_article((title, text)):
    ...
    return title.encode('utf8'), gensim.utils.simple_preprocess(text)  # tokenize


def convert_wiki(infile):
    """
    Yield articles from a bz2 Wikipedia dump `infile` as (title, tokens) 2-tuple.
    ...
    # is dumb and would try to load the entire input into RAM...
    ...
    for group in gensim.utils.chunkize(texts, chunksize=10 * PROCESSES):
        for title, tokens in pool.imap(process_article, group):
            if articles_all % 100000 == 0:
                logger.info("PROGRESS: at article #%i; accepted %i articles with %i total tokens" %
                    (articles_all, articles, positions))
            articles_all += 1
